...
Point it at your own coordinates:
    gen_scene.py --lat 35.99 --lon -78.90 --tz America/New_York
...
DEFAULT_STATE = ".github/scene-state"
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        prog="gen_scene.py",
        description="Render a weather-synced sky strip and seasonal garden footer as animated SVG.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--lat", type=float, default=env("SCENE_LAT", float, DEFAULT_LAT),
                   help="latitude for the weather lookup")
    p.add_argument("--lon", type=float, default=env("SCENE_LON", float, DEFAULT_LON),
    ...
    p.add_argument("--season", choices=SEASONS,
                   help="pin the season instead of deriving it from the date")
    p.add_argument("--out-dir", type=Path, default=Path(env("SCENE_OUT_DIR", str, ".")),
                   help="directory the four SVGs are written to")
    p.add_argument("--state", type=Path, default=Path(env("SCENE_STATE", str, DEFAULT_STATE)),
...
def resolve_weather(args, previous):
...
def main(argv=None):
    ...
    try:
        previous = args.state.read_text().strip()
    except OSError:
        previous = ""
